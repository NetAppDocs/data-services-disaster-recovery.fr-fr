...
* *Différez la resynchronisation SnapMirror et rétablissez la réplication à la demande :* Différez les resynchronisations SnapMirror pour éviter un impact sur les charges de travail de production pendant les périodes de pointe en utilisant l’option « skip protect », puis rétablissez la réplication à la demande avec l’option « re-protect ».
...
Lors de la création d'un plan de réplication dans Disaster Recovery, vous pouvez désormais exclure certains datastores de la réplication. Pour les situations où une machine virtuelle est configurée pour utiliser plusieurs datastores, cela vous permet de limiter ceux qui sont répliqués.
...
La fonctionnalité de reprise après sinistre inclut désormais de nouvelles options de menu permettant de déclencher des actualisations vCenter depuis le tableau de bord du plan de réplication. Vous pouvez accéder au sous-menu depuis le menu Actions du tableau de bord du plan de réplication : sélectionnez **État du plan** puis **Actualiser la source vCenter / Actualiser la cible vCenter**.
...
Vous pouvez désormais accéder à la documentation Swagger directement depuis Disaster Recovery. Dans Disaster Recovery, sélectionnez **Paramètres** puis **Documentation API**. Vous devez être en mode privé ou navigation privée pour accéder à link:https://api.bluexp.netapp.com/v1/services/draas/api/api-doc/draas["le lien"^].
...
Lors de la création d'un plan de réplication, la structure des dossiers de la machine virtuelle est désormais conservée par défaut. Si la cible de récupération ne possède pas l'arborescence de dossiers d'origine, Disaster Recovery la crée. Vous pouvez désélectionner cette option pour ignorer l'arborescence de dossiers d'origine.
...
La NetApp Console, construite sur la fondation BlueXP améliorée et restructurée, assure une gestion centralisée du stockage NetApp et des NetApp Data Services dans les environnements sur site et cloud au niveau entreprise, offrant des informations en temps réel, des flux de travail plus rapides et une administration simplifiée, le tout hautement sécurisé et conforme.
...
La solution Windows Local Administrator Password Solution (Windows LAPS) est une fonctionnalité Windows qui gère et sauvegarde automatiquement le mot de passe d'un compte d'administrateur local sur Active Directory.
...
Pour plus de détails sur la protection contre le saut, consultez https://docs.netapp.com/us-en/data-services-disaster-recovery/use/failover.html["Basculer les applications vers un site distant"].
...
Cette version de NetApp Disaster Recovery prend en charge ASA r2 pour les machines virtuelles et les datastores sur stockage VMFS. Sur un système ASA r2, le logiciel ONTAP prend en charge la fonctionnalité SAN essentielle tout en supprimant les fonctionnalités non prises en charge dans les environnements SAN.
...
Lors de la création d'un plan de réplication, vous pouvez désormais ajouter un préfixe et un suffixe aux noms des machines virtuelles sur le site de reprise après sinistre. Cela vous permet d'utiliser un nom plus descriptif pour les machines virtuelles du plan.
