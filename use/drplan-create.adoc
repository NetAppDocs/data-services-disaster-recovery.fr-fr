...
[role="lead"]
Une fois que vous avez ajouté des ressources et des sites, vous êtes prêt à créer un . Les plans de réplication gèrent la protection des données. La création de la réplication implique de désigner les sites source et de destination, de sélectionner les groupes de ressources et de choisir comment les applications doivent être restaurées et mises sous tension. Par exemple, vous pouvez regrouper des machines virtuelles (VM) associées à une application ou regrouper des applications ayant des niveaux similaires.
...
** Si le groupe de ressources et ses ressources associées doivent basculer vers le site de reprise d'activité, les ressources non protégées n'existeront plus sur le site source après le processus de basculement, ce qui entraînera l'échec des machines virtuelles non protégées sur le site source. De plus, NetApp Disaster Recovery ne démarrera pas ces machines virtuelles non protégées sur le site vCenter de basculement.
...
.Étapes
. Si vous avez déjà des groupes de ressources, sélectionnez les groupes dans la liste, puis.
+
