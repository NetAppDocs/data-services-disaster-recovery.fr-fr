...
. Dans le menu NetApp Disaster Recovery, sélectionnez *Replication plans* puis sélectionnez *Add*. Si c'est la première fois que vous créez un plan de réplication, depuis le Dashboard, sélectionnez *Add replication plan*.
. Saisissez un nom pour le plan de réplication.
. Sous *Source vCenter*, sélectionnez le vCenter où se trouvent les données dans le menu déroulant. Sous *Cible vCenter*, sélectionnez le vCenter vers lequel répliquer les données dans le menu déroulant.
+
...
* Assurez-vous qu'au moins une machine virtuelle est présente dans la banque de données. S'il n'y a aucune machine virtuelle dans la banque de données, la banque de données ne sera pas détectée.
* Un seul magasin de données ne doit pas héberger de machines virtuelles protégées par plusieurs plans de réplication.
* N’hébergez pas de machines virtuelles protégées et non protégées sur le même datastore. Si des machines virtuelles protégées et non protégées sont hébergées sur le même datastore, vous pouvez rencontrer les problèmes suivants :
+
** La capacité utilisée de ce volume peut contribuer au calcul de la licence car Disaster Recovery utilise SnapMirror, ce qui signifie que le système réplique des volumes ONTAP entiers. Dans ce cas, l’espace de volume consommé par les machines virtuelles protégées et non protégées serait inclus dans ce calcul.
...
* *Processeur* et *Mémoire (Go)* : Dans les détails des machines virtuelles, vous pouvez éventuellement redimensionner le processeur et la mémoire de la machine virtuelle.
+
