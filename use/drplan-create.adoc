...
[role="lead"]
Après avoir ajouté des ressources et des sites, créez un plan de réplication pour protéger vos données. Le plan définit les sites source et de destination, les groupes de ressources, les mappages de récupération et le comportement de mise sous tension de vos applications. Vous pouvez également configurer les planifications de conformité et de test et effectuer des basculements de test sans affecter les données de production.
...
** Si des instantanés sont manquants sur le site de destination, le service peut supprimer les instantanés correspondants sur le site source lors du prochain processus de réconciliation des instantanés planifié, en fonction de la politique SnapMirror par défaut pour la relation.
...
=== Considérations relatives aux sauvegardes gérées par la plateforme

Si vous prévoyez d'utiliser des sauvegardes gérées par la plateforme (c'est-à-dire des instantanés gérés par ONTAP), il est recommandé d'utiliser la stratégie de réplication *MirrorAll*. Consultez le tableau pour comprendre l'impact des différentes stratégies de réplication SnapMirror et pourquoi *MirrorAll* est l'option recommandée.

[cols="10,10,80"]
|===


| Politique | Recommandation | Remarques 


| *MirrorAll* | Recommandé | N’utilisez pas les instantanés de transfert SnapMirror pour les opérations de basculement ou de restauration. 


| *MirrorVault* .2+| Recommandation alternative .2+| Assurez-vous que les instantanés sélectionnés lors du basculement ou du retour arrière sont présents sur les volumes source et de destination. Dans le cas contraire, l’erreur suivante s’affichera : « Transition vers l’état ‘snapmirrored’ échouée. Motif : Aucun instantané commun trouvé. » N’utilisez pas les instantanés de transfert SnapMirror pour les opérations de basculement ou de restauration. 


| *Asynchrone* 


| *MirrorLatest* | _NON_ recommandé | Cette politique a pour conséquence que la destination ne dispose en réalité que d'un seul instantané commun pour le basculement, ce qui est sous-optimal pour les scénarios de reprise après sinistre. 
|===


[[considerations-for-disaster-recovery-managed-backups]]
=== Considérations relatives aux sauvegardes gérées par NetApp Disaster Recovery

Lors de l'utilisation de sauvegardes gérées par la reprise après sinistre, les directives suivantes s'appliquent aux relations SnapMirror créées en dehors de NetApp Disaster Recovery :

* Les relations SnapMirror peuvent prendre en charge les politiques de réplication MirrorAll, MirrorVault, MirrorLatest et asynchrone.
* Ne planifiez *pas* de mises à jour pour ces relations, car elles seront gérées par le service NetApp Disaster Recovery.
...
TIP: Organisez vos machines virtuelles ou clusters Kubernetes avant de déployer NetApp Disaster Recovery afin de minimiser la prolifération. Placez les ressources nécessitant une protection sur un sous-ensemble de banques de données et placez les ressources qui ne seront pas protégées sur un autre sous-ensemble de banques de données. Pour organiser les groupes de ressources avant de créer le plan de réplication, consultez link:resource-group.html["Créer un groupe de ressources"].
...
L'assistant vous guide à travers ces étapes :
...
+
image:dr-plan-create-name2.png["Capture d'écran montrant la page du serveur vCenter"]
...
* N’hébergez pas de ressources protégées et non protégées sur le même datastore. Si des ressources protégées et non protégées sont hébergées sur le même datastore, vous pouvez rencontrer les problèmes suivants :
+
...
** Si le groupe de ressources et ses ressources associées doivent être basculés vers le site de reprise d'activité après sinistre, toutes les ressources non protégées n'existeront plus sur le site source à la suite du processus de basculement, ce qui entraînera l'échec des ressources non protégées sur le site source. De plus, NetApp Disaster Recovery ne démarrera pas ces ressources non protégées sur le site de basculement.





TIP: Créez un ensemble de mappages dédié et distinct pour vos tests de basculement afin d'empêcher que les ressources soient connectées aux réseaux de production en utilisant les mêmes adresses IP.
...
+
Si vous n'avez pas de groupes de ressources existants ou si vous devez ajouter des ressources à un groupe, sélectionnez *+ Créer un nouveau groupe de ressources*. Suivez les instructions pour link:resource-group.html["ajouter des espaces de noms à un groupe de ressources"].
...
TIP: Exécutez la commande CLI `kubectl get pvc -n <namespace>` sur l'espace de noms pour afficher la classe de stockage.
...
+
Lors de la configuration de la fréquence des instantanés, vous devez choisir une fréquence d'au moins cinq minutes.
...
+
Pour Azure, choisissez l’*abonnement*, le *groupe de ressources* et le *compte de stockage* pour le fournisseur de cloud.

+
Pour AWS, ONTAP S3 et StorageGRID, choisissez le *Compte*, les *Identifiants* et le *Compartiment*.
...
.. Sélectionnez **Ajouter**.
+
image:kubernetes-mapping.png["Capture d'écran du mappage des classes de stockage pour Kubernetes"]
...
==== Transformations de ressources et exécutions de scripts

Pour les plans de réplication Kubernetes, vous pouvez appliquer des transformations de ressources et exécuter des scripts. Ces étapes sont facultatives.

.Ajouter une transformation de ressource
L'ajout d'une transformation de ressources vous permet de modifier n'importe quel aspect d'une charge de travail lors d'un basculement. Les transformations de ressources offrent la flexibilité nécessaire pour adapter les charges de travail à différents environnements et configurations lors d'une reprise après sinistre. link:task-resource-transformation.html["Découvrez comment créer et approvisionner des transformations."^]

Les transformations de ressources sont appliquées dans l'ordre où vous les indiquez.

. Dans la liste déroulante, sélectionnez la transformation de ressource que vous souhaitez ajouter.
. Choisissez les ressources auxquelles vous souhaitez appliquer la transformation, puis ajoutez les valeurs requises
. Vérifiez l'ordre des transformations pour vous assurer que les opérations s'exécuteront dans le bon ordre.


.Ajouter une exécution de script
NetApp Disaster Recovery peut exécuter un script avant ou après la prise d'un instantané afin de gérer la mise en veille et la reprise d'une application. Cette action garantit que les applications sont correctement préparées avant qu'un instantané ne soit répliqué et restauré, et qu'elles peuvent reprendre leurs opérations sans interruption après un basculement.

. Sélectionnez *+ Add script*.
. Sélectionnez *Téléverser* pour choisir le fichier de script.
. Choisissez l’*étape d’exécution* : *Pré-instantané* pour exécuter avant la création de l’instantané ou *Post-instantané* pour exécuter après.
...
La section Ressources de calcul définit l'emplacement de restauration des machines virtuelles après un basculement. Associez le centre de données et le cluster vCenter source au centre de données et au cluster cibles.

Il est possible de redémarrer les machines virtuelles sur un hôte ESXi vCenter spécifique. Si VMware DRS est activé, vous pouvez déplacer la machine virtuelle automatiquement vers un autre hôte si nécessaire pour respecter la stratégie de reprise après sinistre configurée.

Vous pouvez également placer toutes les machines virtuelles de ce plan de réplication dans un dossier unique au sein de l’inventaire vCenter. Cela vous permet d’organiser rapidement les machines virtuelles basculées dans le vCenter.
...
NOTE: Cette section est uniquement requise pour vCenters. Vous n'avez pas besoin de la remplir pour Kubernetes.
...
Dans l'onglet Mappages de basculement, sélectionnez la flèche vers le bas en regard de *Réseaux virtuels*. Sélectionnez le réseau virtuel source et le réseau virtuel cible.
...
NOTE: Activez les *paramètres avancés* pour configurer les *scripts* et la *cohérence au niveau des applications*.
...
* *Type d'adresse IP* : reconfigurez la configuration de la machine virtuelle pour qu'elle corresponde aux exigences du réseau virtuel cible. NetApp Disaster Recovery propose deux options : DHCP ou IP statique. Pour les IP statiques, configurez le masque de sous-réseau, la passerelle et les serveurs DNS. Saisissez également les informations d'identification pour les machines virtuelles.
+
...
+
*Machines virtuelles exécutant Microsoft Windows* : peuvent exécuter soit des scripts batch Windows (.bat), soit des scripts PowerShell (ps1). Les scripts Windows peuvent utiliser des arguments de ligne de commande. Formatez chaque argument au format  `arg_name$value` où  `arg_name` est le nom de l’argument et  `$value` est la valeur de l’argument, et un point-virgule sépare chaque paire  `argument$value`.
...
+
** Démarrage séquentiel : attribuez à chaque VM un numéro unique pour les démarrer dans l’ordre attribué, par exemple, 1,2,3,4,5.
...
image:dr-plan-resource-mapping-vm-boot-order.png["Capture d'écran montrant la section des machines virtuelles des mappages de ressources"]
...
Les données gérées par le serveur de base de données peuvent se trouver sur le même datastore que la machine virtuelle qui héberge le serveur de base de données, ou sur un datastore différent. Le tableau suivant présente les configurations prises en charge pour les snapshots cohérents au niveau des applications dans NetApp Disaster Recovery :
...
* Pour les machines virtuelles Windows, le service utilise le service Microsoft Volume Shadow Copy (VSS) pour se coordonner avec chaque type de serveur de base de données.
...
TIP: Examinez le <<Considérations relatives aux sauvegardes gérées par la plateforme>> avant de faire ces sélections.

* *Utilisez les sauvegardes et les calendriers de rétention gérés par la plateforme* : Si vous utilisez une solution externe de gestion des snapshots, cochez cette case. La reprise après sinistre prend en charge l’utilisation de solutions externes de gestion des snapshots, telles que le planificateur de stratégies natif ONTAP SnapMirror ou les intégrations tierces. Si chaque datastore (volume) du plan de réplication possède déjà une relation SnapMirror gérée ailleurs, vous pouvez utiliser ces snapshots comme points de restauration dans NetApp Disaster Recovery.
+
Lorsque cette option est sélectionnée, NetApp Disaster Recovery ne configure pas de planification de sauvegarde. Cependant, vous devez tout de même configurer une planification de rétention, car des instantanés peuvent toujours être pris pour des opérations de test, de basculement et de restauration après basculement.
...
* *Magasins de données source et cible* : Si plusieurs relations SnapMirror (de distribution) existent, vous pouvez sélectionner la destination à utiliser. Si une relation SnapMirror est déjà établie pour un volume, les magasins de données source et cible correspondants s’affichent. Si un volume n’a pas de relation SnapMirror, vous pouvez en créer une maintenant en sélectionnant un cluster de destination, une SVM de destination et en indiquant un nom de volume. Le service créera le volume et la relation SnapMirror.
+
...
** Lors du basculement, vous pouvez sélectionner n'importe quel instantané. Si vous sélectionnez le dernier instantané, le service crée une sauvegarde à la demande, met à jour la destination et utilise cet instantané pour le basculement.
