...
Avant de procéder à un basculement en direct, vous pouvez effectuer un test pour vérifier votre préparation. Les charges de travail restent en ligne pendant le processus de test.
...
+
Saisissez ensuite « Test failover » et sélectionnez *Test fail over*.
...
IMPORTANT: Si le plan de réplication inclut une banque de données vide, vous devez d'abord supprimer la banque de données vide du plan ou utiliser l'option **Forcer le basculement**, qui ignore les banques de données vides.
...
. Sélectionnez *Forcer le basculement* si vous souhaitez que le basculement se produise même si une erreur est détectée qui empêcherait normalement le basculement de se produire.
