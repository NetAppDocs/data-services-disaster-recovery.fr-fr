...
[role="lead"]
Une fois les sites de ressources ajoutés, vous pouvez créer des groupes de ressources pour regrouper des machines virtuelles, des banques de données ou des clusters Kubernetes en une seule unité. Les groupes de ressources vous permettent d'organiser un ensemble de ressources dépendantes en groupes logiques répondant à vos besoins. Par exemple, vous pouvez regrouper les machines virtuelles associées à une même application ou regrouper des applications ayant des niveaux similaires. Autre exemple : les groupes peuvent contenir des ordres de démarrage différé qui peuvent être exécutés lors d'une reprise.
...
* Lors de la création d'un plan de reprise après sinistre ou de réplication. Si vous avez de nombreuses machines virtuelles hébergées par un cluster vCenter source, il peut être plus simple de créer les groupes de ressources pendant la création du plan de réplication. Pour obtenir des instructions sur la création de groupes de ressources lors de la création d'un plan de réplication, consultez link:drplan-create.html["Créer un plan de réplication"].
...
. Dans le menu déroulant, filtrez par groupes de ressources *vCenter*.
+
...
.Étapes
. Dans la section Disaster Recovery, sélectionnez **Groupes de ressources** dans la barre latérale.
. Dans le menu déroulant, filtrez par sites *Kubernetes*.
+
image::site-type-toggle.png[Capture d'écran du bouton de basculement du type de site]
...
+
image:kubernetes-resource-group.png["Ajouter un groupe de ressources Kubernetes"]
