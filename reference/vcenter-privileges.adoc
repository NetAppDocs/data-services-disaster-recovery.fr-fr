...
[role="lead"]
Le compte vCenter pour NetApp Disaster Recovery doit disposer d’un ensemble minimal de privilèges pour enregistrer et désenregistrer des datastores, démarrer et arrêter des machines virtuelles (VM) et reconfigurer des VM.

Le tableau suivant répertorie les privilèges requis par la fonction de récupération après sinistre pour fonctionner avec un cluster vCenter.
...
.2+| Hôte | Host.Config.AdvancedConfig | Paramètres avancés | Permet de définir des options avancées de configuration de l'hôte. 


| Host.Config.Stockage | Configuration de la partition de stockage | Permet la numérisation des nouveaux périphériques de stockage et la gestion de l’iSCSI. 


| Ressource | Resource.AssignVMToPool | Pools de ressources | Permet l'affectation de machines virtuelles à un pool de ressources 
...
.12+| Configuration de la machine virtuelle | VirtualMachine.Config.AdvancedConfig | Configuration avancée | Permet d'ajouter ou de modifier des paramètres avancés dans le fichier de configuration de la VM. 


| VirtualMachine.Config.AddExistingDisk | Ajouter un disque existant | Permet d'ajouter un disque virtuel existant à une VM. 


| VirtualMachine.Config.AddNewDisk | Ajouter un nouveau disque | Permet de créer un nouveau disque virtuel à ajouter à une machine virtuelle. 


| VirtualMachine.Config.RemoveDisk | Retirer le disque | Permet la suppression d'un périphérique de disque virtuel. 
