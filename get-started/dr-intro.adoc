...
* Amazon Elastic VMware Service (EVS) avec Amazon FSx for NetApp ONTAP. Pour plus de détails, consultez link:../reference/evs-deploy-guide-introduction.html["Introduction à la reprise après sinistre NetApp avec Amazon Elastic VMware Service et Amazon FSx for NetApp ONTAP"].
* VMware Cloud (VMC) sur AWS avec Amazon FSx for NetApp ONTAP
* Azure VMware Solution avec NetApp Cloud Volumes ONTAP (iSCSI) (Aperçu privé)
...
* Achetez un abonnement à la carte (PAYGO) sur la Marketplace Amazon Web Services (AWS), la Marketplace Azure ou la Marketplace Google Cloud. Cette licence vous permet d’acquérir une licence de capacité protégée fixe sans engagement à long terme.
...
NOTE: NetApp Disaster Recovery facture en fonction de la capacité utilisée des datastores sur le site source lorsqu'il existe au moins une machine virtuelle ou une ressource Kubernetes disposant d'un plan de réplication. La capacité d'un datastore basculé n'est pas incluse dans l'allocation de capacité. Pour un BYOL, si le volume de données dépasse la capacité autorisée, les opérations dans le service sont limitées jusqu'à l'obtention d'une licence de capacité supplémentaire ou la mise à niveau de la licence dans la NetApp Console.
...
* VMware Cloud (VMC) sur AWS avec Amazon FSx for NetApp ONTAP
* Un autre environnement VMware sur site, basé sur NFS avec un stockage ONTAP, ou un environnement VMFS FC/iSCSI sur site
