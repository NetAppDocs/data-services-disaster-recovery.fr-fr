...
[role="lead"]
La reprise après sinistre dans le cloud est une solution résiliente et économique pour protéger les charges de travail contre les pannes de site et les événements de corruption des données. Avec NetApp Disaster Recovery, vous pouvez répliquer vos charges de travail VMware VM ou datastore sur site exécutant un stockage ONTAP vers un centre de données VMware défini par logiciel dans un cloud public en utilisant le stockage cloud NetApp ou vers un autre environnement VMware sur site avec stockage ONTAP en tant que site de reprise après sinistre. Vous pouvez également utiliser Disaster Recovery pour migrer des charges de travail de machines virtuelles d’un site à un autre.
...
La reprise après sinistre utilise les composants suivants pour assurer les charges de travail de reprise après sinistre :
