...
[role="lead"]
Pour utiliser NetApp Disaster Recovery, effectuez quelques étapes de configuration dans la NetApp Console et dans votre environnement de reprise après sinistre pris en charge.
...
* Reprise après sinistre dans le cloud hybride qui réplique un centre de données VMware plus ONTAP sur site vers une infrastructure de reprise après sinistre AWS basée sur VMware Cloud on AWS ou Elastic VMware Service (EVS) et Amazon FSx for NetApp ONTAP.
* Cloud privé DR qui réplique un VMware plus ONTAP vCenter sur site vers un autre VMware plus ONTAP vCenter sur site.
* Reprise après sinistre dans le cloud qui réplique une infrastructure de reprise après sinistre AWS basée sur VMware Cloud on AWS ou EVS vers une autre infrastructure de reprise après sinistre basée sur AWS utilisant FSx pour NetApp ONTAP.
* Reprise après sinistre dans le cloud hybride qui réplique un centre de données VMware plus ONTAP sur site vers une infrastructure de reprise après sinistre Google basée sur Google Cloud VMware Engine et Google Cloud NetApp Volumes.
...
Les sites de production et de reprise après sinistre sont reliés par une connexion sécurisée compatible avec AWS. Les types de connexion courants incluent un VPN sécurisé (privé ou fourni par AWS), AWS Direct Connect ou d'autres méthodes d'interconnexion approuvées.
...
* *AWS Site-to-Site VPN* : Cette connexion de réseau privé virtuel peut fournir une connectivité sécurisée via un fournisseur d'accès Internet public. Le VPN chiffre et déchiffre toutes les données transitant vers et depuis le cloud AWS. Les VPN peuvent être basés sur des logiciels ou du matériel. Pour les applications d'entreprise, le fournisseur d'accès Internet (FAI) public doit offrir des garanties de qualité de service afin d'assurer une bande passante et une latence adéquates pour la réplication de reprise après sinistre.
...
*MEILLEURE PRATIQUE* : Pour les solutions de reprise après sinistre utilisant VMware Cloud et un seul FSx for ONTAP VPC, NetApp recommande d’utiliser le peering VPC. Si plusieurs FSx for ONTAP VPC sont déployés, nous recommandons d’utiliser une AWS Transit Gateway afin de réduire la charge de gestion liée à la multiplication des connexions de peering VPC.
...
La récupération après sinistre prend en charge la réplication de site à site à l'aide d'Ethernet et de TCP/IP.  Assurez-vous qu'une bande passante adéquate est disponible pour prendre en charge les taux de modification des données sur les machines virtuelles du site de production afin que toutes les modifications puissent être répliquées sur le site DR dans le délai de l'objectif de point de récupération (RPO).
