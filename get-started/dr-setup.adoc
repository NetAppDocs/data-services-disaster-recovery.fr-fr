...
[role="lead"]
Pour utiliser NetApp Disaster Recovery, effectuez quelques étapes pour la configurer à la fois dans la NetApp Console et dans l'environnement de reprise après sinistre pris en charge.
...
* *VPN site à site AWS* : Cette connexion de réseau privé virtuel peut être utilisée pour fournir des connexions d'accès sécurisées ainsi qu'un service de fournisseur d'accès Internet public. Le VPN chiffre et déchiffre toutes les données transitant vers et depuis le cloud AWS. Les VPN peuvent être basés sur des logiciels ou du matériel. Pour les applications d’entreprise, le fournisseur d’accès Internet (ISP) doit offrir des garanties de qualité de service afin d’assurer une bande passante et une latence adéquates pour la réplication DR.
